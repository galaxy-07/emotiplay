...
import { Play, Pause, SkipForward, SkipBack, Volume2, Music, ListMusic, Loader2 } from 'lucide-react';
  ...
  return <Card className={`w-full backdrop-blur-md bg-card/95 border-accent/30 ${className || ''}`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-center gap-2">
          <Button variant="outline" size="icon" onClick={playPrevious} disabled={queue.tracks.length === 0}>
            <SkipBack className="h-4 w-4" />
          </Button>
          
          <Button size="icon" onClick={togglePlayPause} disabled={!currentTrack || isLoading} className="h-12 w-12">
            {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : isPlaying ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5" />}
          </Button>
          
          <Button variant="outline" size="icon" onClick={playNext} disabled={queue.tracks.length === 0}>
        ...
        {/* Queue Display */}
        {queue.tracks.length > 0 && <div className="space-y-2">
            <div className="flex items-center gap-2">
            ...
            <ScrollArea className="h-32 w-full">
              <div className="space-y-1">
                {queue.tracks.map((track, index) => <div key={track.id} onClick={() => playTrackFromQueue(index)} className={`flex items-center gap-2 p-2 rounded text-sm cursor-pointer hover:bg-muted/50 transition-colors ${index === queue.currentIndex ? 'bg-primary/10 border border-primary/20' : 'bg-muted/30'}`}>
                    ...
                    {index === queue.currentIndex && <Badge variant="secondary" className="text-xs">
